...
def ensure_legacy_columns():
    ...
    try:
        ...
        with engine.begin() as conn:
            for table in ("orders", "expenses", "supplier_payments"):
                if insp.has_table(table) and "user_id" not in {c["name"] for c in insp.get_columns(table)}:
...
def send_order_to_steadfast(order_id):
    ...
    try:
        ...
        cons=result.get("consignment") or {}
        o.consignment_id=clean(cons.get("consignment_id"));o.tracking_code=clean(cons.get("tracking_code"));o.steadfast_status=clean(cons.get("status"));o.status="submitted"
...
def sync_order(order_id):
    ...
    try:
        ...
        cons = result.get("consignment") or {}
        s = cons.get("status") or result.get("status")
        if s:
            o.steadfast_status = clean(s)
            o.status = map_status(s)
        if cons.get("tracking_code"): o.tracking_code = clean(cons.get("tracking_code"))
        if cons.get("consignment_id"): o.consignment_id = clean(cons.get("consignment_id"))
