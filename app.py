...
import os, re, logging, secrets, json, base64
...
GOOGLE_SHEETS_ENABLED = os.getenv("GOOGLE_SHEETS_ENABLED", "true").strip().lower() not in ("0", "false", "no", "off")
...
def _google_service():
    if not GOOGLE_SHEETS_ENABLED or not GOOGLE_SHEET_ID:
        return None
    if not (GOOGLE_SERVICE_ACCOUNT_JSON or GOOGLE_SERVICE_ACCOUNT_BASE64):
        return None
    if GoogleCredentials is None or google_build is None:
        logging.warning("Google Sheets libraries are not installed.")
        return None
    try:
        raw = GOOGLE_SERVICE_ACCOUNT_JSON
        if not raw:
            raw = base64.b64decode(GOOGLE_SERVICE_ACCOUNT_BASE64).decode("utf-8")
        info = json.loads(raw)
        creds = GoogleCredentials.from_service_account_info(info, scopes=GOOGLE_SCOPES)
        return google_build("sheets", "v4", credentials=creds, cache_discovery=False)
    except Exception:
        logging.exception("Google Sheets service initialization failed")
        return None

def _google_user_label(db):
    uid = current_user_id()
    if not uid:
        return ""
    try:
...
def _sheet_values(service, range_name):
    return service.spreadsheets().values().get(
        spreadsheetId=GOOGLE_SHEET_ID, range=range_name
    ).execute().get("values", [])

def _ensure_sheet_tabs(service):
    meta = service.spreadsheets().get(spreadsheetId=GOOGLE_SHEET_ID).execute()
    existing = {s["properties"]["title"] for s in meta.get("sheets", [])}
    needed = ["Orders", "Expenses", "Supplier Payments"]
    requests_body = [
        {"addSheet": {"properties": {"title": title}}}
        for title in needed if title not in existing
    ]
    if requests_body:
    ...
    headers_map = {
        "Orders": ["App User","Order ID","Invoice","Created At","Updated At","Customer Name","Phone",
                   "Alternative Phone","Address","District","Thana","COD","Supplier","Supplier Cost",
                   "Delivery Charge","Quantity","Weight","Item Description","Note","Exchange",
                   "App Status","Steadfast Status","CN# / Parcel ID","Tracking ID"],
        "Expenses": ["App User","Expense ID","Created At","Title","Amount","Note"],
        "Supplier Payments": ["App User","Payment ID","Created At","Supplier","Amount","Note"],
    }
    for title, headers in headers_map.items():
        vals = _sheet_values(service, f"'{title}'!A1:Z1")
...
def _find_row_by_key(service, title, key_column, key):
    rows = _sheet_values(service, f"'{title}'!A2:Z")
    idx = key_column - 1
    for row_no, row in enumerate(rows, start=2):
        if len(row) > idx and str(row[idx]) == str(key):
            return row_no
    return None

def _upsert_sheet_row(service, title, key_column, key, values):
    row_no = _find_row_by_key(service, title, key_column, key)
    ...
    if row_no is None:
        ...
        service.spreadsheets().values().update(
            spreadsheetId=GOOGLE_SHEET_ID,
            range=f"'{title}'!A{row_no}:{end_col}{row_no}",
            valueInputOption="RAW",
            body={"values": [values]}
        ).execute()

def sync_order_to_google(o, db):
    service = _google_service()
    if not service:
        return
    try:
        _ensure_sheet_tabs(service)
        values = [
            _google_user_label(db), o.id, o.invoice,
            o.created_at.isoformat() if o.created_at else "",
            o.updated_at.isoformat() if o.updated_at else "",
            o.customer_name, o.phone, o.alternative_phone or "", o.address,
            o.district, o.thana, o.cod_amount, o.supplier_name or "", o.supplier_cost,
            o.delivery_charge, o.quantity, o.weight, o.item_description or "", o.note or "",
            "Yes" if o.exchange else "No", o.status, o.steadfast_status or "",
            o.consignment_id or "", o.tracking_code or ""
        ]
        _upsert_sheet_row(service, "Orders", 2, o.id, values)
    except Exception:
        logging.exception("Google Sheets order sync failed")

def sync_expense_to_google(x, db):
    service = _google_service()
    if not service:
        return
    try:
        _ensure_sheet_tabs(service)
        values = [
            _google_user_label(db), x.id,
            x.created_at.isoformat() if x.created_at else "",
            x.title, x.amount, x.note or ""
        ]
        _upsert_sheet_row(service, "Expenses", 2, x.id, values)
    except Exception:
        logging.exception("Google Sheets expense sync failed")

def sync_payment_to_google(x, db):
    service = _google_service()
    if not service:
        return
    try:
        ...
        values = [
            _google_user_label(db), x.id,
            x.created_at.isoformat() if x.created_at else "",
            x.supplier_name, x.amount, x.note or ""
        ]
        _upsert_sheet_row(service, "Supplier Payments", 2, x.id, values)
    except Exception:
        logging.exception("Google Sheets payment sync failed")
...
def edit_saved_order(order_id):
    ...
    try:
        ...
        db.commit();db.refresh(o);sync_order_to_google(o, db);return jsonify({"ok":True,"order":order_dict(o),"message":"Saved order update হয়েছে।"})
    except Exception as e:
...
def add_expense():
    ...
    try:
        ...
        sync_expense_to_google(x, db) if isinstance(x, Expense) else sync_payment_to_google(x, db)
...
def google_sheets_status():
    if (x := require_login()): return x
    configured = bool(GOOGLE_SHEET_ID and (GOOGLE_SERVICE_ACCOUNT_JSON or GOOGLE_SERVICE_ACCOUNT_BASE64))
    enabled = bool(GOOGLE_SHEETS_ENABLED and configured and google_build)
    return jsonify({
        "ok": True,
        "configured": configured,
        "enabled": enabled,
        "spreadsheet_id": GOOGLE_SHEET_ID,
        "message": "Google Sheets sync is ready." if enabled else "Railway Variables-এ GOOGLE_SHEET_ID এবং GOOGLE_SERVICE_ACCOUNT_JSON সেট করুন।"
    })
...
def download_backup():
    ...
    try:
        payload = {
            ...
            "orders": [order_dict(o) for o in db.query(Order).filter(Order.user_id == current_user_id()).order_by(Order.id.asc()).all()],
            "expenses": [
                {"id": x.id, "title": x.title, "amount": x.amount,
                 "note": x.note or "", "created_at": x.created_at.isoformat() if x.created_at else ""}
                for x in db.query(Expense).filter(Expense.user_id == current_user_id()).order_by(Expense.id.asc()).all()
            ],
            "supplier_payments": [
                {"id": x.id, "supplier_name": x.supplier_name, "amount": x.amount,
                 "note": x.note or "", "created_at": x.created_at.isoformat() if x.created_at else ""}
                for x in db.query(SupplierPayment).filter(SupplierPayment.user_id == current_user_id()).order_by(SupplierPayment.id.asc()).all()
            ],
        }
...
@app.patch("/api/expenses/<int:item_id>")
def update_expense(item_id):
    if (x := require_login()): return x
    db = SessionLocal()
    try:
        x = db.query(Expense).get(item_id)
        if not x: return jsonify({"ok": False, "message": "Expense not found"}), 404
        data = request.get_json(silent=True) or {}
        if "title" in data: x.title = data["title"]
        if "amount" in data: x.amount = float(data["amount"])
        if "note" in data: x.note = data["note"]
        db.commit(); db.refresh(x)
        sync_expense_to_google(x, db)
        return jsonify({"ok": True})
    except Exception as e:
        db.rollback()
        return jsonify({"ok": False, "message": "Expense update failed", "details": str(e)}), 500
    finally: db.close()
...
def delete_expense(item_id):
    ...
    try:
        ...
        db.rollback()
        return jsonify({"ok": False, "message": "Expense delete failed", "details": str(e)}), 500
    finally: db.close()

@app.patch("/api/supplier-payments/<int:item_id>")
def update_supplier_payment(item_id):
    if (x := require_login()): return x
    db = SessionLocal()
    try:
        x = db.query(SupplierPayment).get(item_id)
        if not x: return jsonify({"ok": False, "message": "Supplier payment not found"}), 404
        data = request.get_json(silent=True) or {}
        if "supplier_name" in data: x.supplier_name = data["supplier_name"]
        if "amount" in data: x.amount = float(data["amount"])
        if "note" in data: x.note = data["note"]
        db.commit(); db.refresh(x)
        sync_payment_to_google(x, db)
        return jsonify({"ok": True})
    except Exception as e:
        db.rollback()
        return jsonify({"ok": False, "message": "Supplier payment update failed", "details": str(e)}), 500
    finally: db.close()

@app.delete("/api/supplier-payments/<int:item_id>")
def delete_supplier_payment(item_id):
    if (x := require_login()): return x
    db = SessionLocal()
    try:
        x = db.query(SupplierPayment).get(item_id)
        ...
        db.rollback()
        return jsonify({"ok": False, "message": "Supplier payment delete failed", "details": str(e)}), 500
    finally: db.close()
